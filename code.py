...
with tf.Session() as tt:
    ...
    for b in range(50):
        a, m = tt.run([loss_cal, mina], feed_dict={X_data: data_x, Y_data: data_y})
        brt = tt.run(pred, feed_dict={X_data: data_x})
        print([main_words[i] for i in np.squeeze(brt)])
    saver.save(tt,'/Users/exepaul/Desktop/tensor/jklop.ckpt')
